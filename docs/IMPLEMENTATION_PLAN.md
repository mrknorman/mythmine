...
- **Creative/survival parity — DONE.** The client-authoritative creative path is scoped to where it
  actually applies: `creativeClient` ops (drag-distribute, expand/contract, unmix) run client-side +
  creative-slot-sync ONLY for the player's own `inventoryMenu` (the creative screen); a creative player
  in a chest/crafting screen uses the normal server round-trip — no more double-execution. Creative
  scroll-select pushes the seeded stack through `handleCreativeModeItemAdd` (the canonical creative
  slot edit) instead of the survival payload. Server-side ops (crafting/transmute, unmix, furnaces,
  recipe book) are gamemode-independent — pinned by creative-mode MenuSelfTest checks.
- **Per-wood stick textures — DONE (programmatic).** Generated by palette-mapping the vanilla stick
  texture through each wood's planks palette (luminance-ranked LUT sampled from the actual planks
  textures) — derived art, no hand-drawn assets. Regenerate with `python3
  scripts/gen_stick_textures.py` (reads the 26.2 client jar from the loom cache).
...
- **Stone phase — S1 COMPLETE (239 blocks: core+mossy+tier-2); S2 families pending.** See `STONE_PHASE.md`: material × form grid —
  ...
  resolved: loot normalization YES (raw drops cobbled, silk touch restores), everything ships on
  generated textures (~80 new bases → ART_DEBT.md). 27 form-groups (brick_stairs never group with
  plain stairs). Staged S1a core → S1b mossy → S1c tier-2 → S2 families.
